...
angular.module('mooc')
	.controller('moocCtrl', function moocCtrl($scope, $routeParams, $filter, agg) {
		...
		//default services
		var defaultServices = ['reddit'];

		$scope.addService = function (service) {
			if ($('#'+service).hasClass('unloaded')&&!_(defaultServices).contains(service)) {
				//do nothing...
		...
		_(defaultServices).each(function (active){
			$scope.addService(active);
		});
